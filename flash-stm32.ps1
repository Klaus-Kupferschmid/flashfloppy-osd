...
Write-Host "`n🔧 Method 1: STM32CubeProgrammer..." -ForegroundColor Yellow
...
if (Test-Path $programmer) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Flash successful with STM32CubeProgrammer!" -ForegroundColor Green
        exit 0
    } else {
        Write-Host "❌ STM32CubeProgrammer failed:" -ForegroundColor Red
        Write-Host "$result" -ForegroundColor DarkRed
    }
} else {
    Write-Host "❌ STM32CubeProgrammer not found!" -ForegroundColor Red
}

# Method 2: Try alternative parameters
Write-Host "`n🔧 Method 2: Alternative ST-Link parameters..." -ForegroundColor Yellow
...
foreach ($params in $alternatives) {
    Write-Host "Trying: $params" -ForegroundColor Cyan
    $result = & $programmer $params.Split(' ') 2>&1
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Flash successful with alternative parameters!" -ForegroundColor Green
        exit 0
    }
}

# Method 3: Information and troubleshooting
Write-Host "`n🔧 Method 3: Troubleshooting Information..." -ForegroundColor Yellow
...
Write-Host "`n💡 Alternative: Use VS Code Debugging" -ForegroundColor Cyan
