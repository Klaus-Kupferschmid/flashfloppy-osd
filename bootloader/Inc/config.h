...
#elif defined TARGET_FLASHFLOPPY_OSD
	/* FlashFloppy OSD Custom Target
	 * LED1: PB2 (active low) - onboard LED
	 * Boot Button: PA4 (Boot-Select, active low with internal pull-up)
	 */
	#define LED1_CLOCK		RCC_APB2ENR_IOPBEN
	/* PB2 as push-pull output, 2MHz: CNF=00, MODE=10 */
	#define LED1_BIT_0		do { \
		CLEAR_BIT(GPIOB->CRL, GPIO_CRL_CNF2); \
		MODIFY_REG(GPIOB->CRL, GPIO_CRL_MODE2, GPIO_CRL_MODE2_1); \
	} while(0)
	#define LED1_BIT_1		/* not used */
	#define LED1_MODE		/* not used */
	#define LED1_ON			WRITE_REG(GPIOB->BRR, GPIO_BRR_BR2)
	#define LED1_OFF		WRITE_REG(GPIOB->BSRR, GPIO_BSRR_BS2)

	/* Boot button on PA4 - use this instead of PB2 (BOOT1) */
	#define BOOT_BUTTON_CLOCK	RCC_APB2ENR_IOPAEN
	#define BOOT_BUTTON_INIT()	do { \
		/* PA4 as input with pull-up: CNF=10 (input pull-up/down), MODE=00 (input) */ \
		CLEAR_BIT(GPIOA->CRL, GPIO_CRL_MODE4); \
		CLEAR_BIT(GPIOA->CRL, GPIO_CRL_CNF4_0); \
		SET_BIT(GPIOA->CRL, GPIO_CRL_CNF4_1); \
		SET_BIT(GPIOA->ODR, GPIO_ODR_ODR4); /* Pull-up */ \
	} while(0)
	#define BOOT_BUTTON_PRESSED()	(READ_BIT(GPIOA->IDR, GPIO_IDR_IDR4) == 0)
