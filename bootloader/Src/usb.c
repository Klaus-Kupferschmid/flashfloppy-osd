...
volatile bool UsbEnumerated;  /* Set when USB gets address, persists through suspend */
...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
	...
	while ((istr = READ_REG(*ISTR) & ISTR_MASK) != 0) {
		...
		if (READ_BIT(istr, ISTR_RESET)) {
			...
			UsbEnumerated = false;  /* Reset enumeration flag */
			USB_Reset();
		}
		...
		if (READ_BIT(istr, ISTR_SUSP)) {
			...
			if (READ_REG(*DADDR) & USB_DADDR_ADD) {
				...
				/* DON'T clear UsbEnumerated here - suspend is not disconnect! */
			}
		}
